...
export function CartDrawerCard( { product, quantity } : CartDrawerCardProps ) {
    ...
    return (
        <S.Card>
            <S.CartDrawerCloseButton onClick={() => removeFromCart(product.id)}>X</S.CartDrawerCloseButton>
            ...
            <S.PriceQuantityContainer>
                ...
                <S.CardPrice>{formatCurrency(Number(product.price) * quantity)}</S.CardPrice>
            </S.PriceQuantityContainer>
        </S.Card>
    )
}